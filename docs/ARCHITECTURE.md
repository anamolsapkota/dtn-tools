...
### History File Format (`~/dtn/chat-history.json`)
...
```json
{
  "conversations": {
    "268485000": {
      "name": "DTNGW",
      "messages": [
        {"dir": "in", "from": "268485000", "name": "DTNGW", "msg": "hello", "ts": "2026-05-14T18:40:10+00:00", "read": true},
        {"dir": "out", "from": "268485091", "name": "pi05", "msg": "hi back", "ts": "2026-05-14T18:40:15+00:00", "read": true}
      ]
    }
  },
...
- Messages capped at 500 per conversation (oldest pruned)
- Atomic write: temp file + `os.replace()` to prevent corruption

### Sending Flow

```
User types message in [node-name] you> prompt
       |
       v
ChatSession.send_bundle() builds JSON payload
       |
       v
bpsource ipn:<dest>.5 '<JSON>'
       |
       v
Message saved to chat-history.json (dir: "out", read: true)
       |
       v
ION CGR computes route -> bundle sent via UDP
```

### Receiving Flow

```
Background receiver_thread starts bprecvfile ipn:<local>.5
       |
       v
...
Receiver thread polls directory every 0.5 seconds
       |
       v
Parses JSON -> routes to correct conversation
       |
       +---> Active sender? Print inline, save as read
       |
       +---> Other sender? Save as unread, print notification:
             "<- echo-dhulikhel sent a message (2 unread)"
```
...
| Command | Action |
|---------|--------|
| `/to <name\|#\|IPN>` | Switch active conversation, show recent history |
| `/list` | Show all conversations with unread counts |
| `/history [N]` | Show last N messages in current conversation |
| `/nodes` | List all nodes from contact graph |
| `/help` | Show command list |
| `/quit` | Exit chat |

### Conversation Flow

1. On startup, load `chat-history.json`
2. Show unread summary if any
3. Offer to resume last active conversation
4. Messages from active sender print inline; others show notification only
5. `/to` switches and marks all messages in target as read
6. On exit, save last active conversation for next session
...
| `discovered_nodes.json` | `~/dtn/dtn-discovery/` | Persistent database of discovered nodes (includes cached outducts for fast recovery) |
| `ipnd.rc` | `~/dtn/dtn-discovery/` | IPND beacon configuration |
| `chat-history.json` | `~/dtn/` | Persistent chat history with per-sender conversations and unread tracking |
